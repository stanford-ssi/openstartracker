"""Star Tracking (Program description TBD after deconstructing this)

TODO:
- pare down this entire file and make it more organized
- fix style to confirm to PEP
    - keep '' and "" consistent
- use argparse rather than sys.argv DONE
...
- figure out what the server and socket stuff is for
"""
...
def parse_args():
    """Parse user-inputted arguments when running this script.
    """
    parser = argparse.ArgumentParser(description='Star Tracker')
    parser.add_argument('-c', '--config', type=str, help='Config file')
    parser.add_argument('-Y', '--year', type=float, help='Year')
    parser.add_argument('-m', '--median', type=str, help='The median image to use')
...
def trace(frame, event, arg):
    """What does this do??? Why does it return itself? TBD"""
    print('{}, {}:{}'.format(event, frame.f_code.co_filename, frame.f_lineno), file=sys.stderr)
    return trace

# The following two functions map back and forth between
...
def a2q(A):
    ...
    Output:
        q: (4,) matrix representing A in quaternion form
    """
...
def q2a(q):
    """Convert a quaternion to an orthogonal 3x3 matrix A.

    Input:
        q: (4,) matrix representing a quaternion
    Output:
...
def extrapolate_matrix(A, B, t1, t2, t3):
    ...
    C = q2a(np.array([0.5 * anglesBC[0],
                      0.5 * anglesBC[1],
                      0.5 * anglesBC[2],
                      1]))

    return (C, 1e6 * anglesAB / (t2 - t1))
...
def print_ori(body2ECI):
    """Print the orientation of an attitude matrix.

    Input:
        body2ECI: attitude matrix
    """
    ...
    print("DEC=".format(DEC), file=sys.stderr)
    #rotation about the z axis (-180 to +180)
    print("RA={}".format(RA), file=sys.stderr)
    #rotation about the camera axis (-180 to +180)
    print("ORIENTATION=".format(ORIENTATION), file=sys.stderr)
    ...
    def print_match(self, body_correction=None, angrate_string=""):
        ...
        for i in range(db.size()):
            ...
            if (s_db.id >= 0):
                ...
                temp = np.dot(body_correction, np.array([[s_im.x], [s_im.y], [s_im.z]]))
                star_out.append('{},{},{},{},{},{},{}'.format(temp[0,0], temp[1,0], temp[2,0],
                                                              s_db.x, s_db.y, s_db.z, weight))

        print("stars", len(star_out), file=sys.stderr)
        print("ang_rate: {}".format(angrate_string), file=sys.stderr)
...
class StarCamera:
    ...
    def solve_image(self, imagefile, lis=1, quiet=0):
        ...
        if (SIMULATE==1 and quiet==0):
            ...
            data = s.recv(2048)
            s.close()
        print("Time1: " + str(time.time() - starttime), file=sys.stderr)
        self.current_image = StarImage(imagefile,self.median_image)
        print("Time2: " + str(time.time() - starttime), file=sys.stderr)
        if lis == 1:
            self.current_image.match_lis()
        print("Time3: "+str(time.time() - starttime), file=sys.stderr)
        ...
        if (quiet==0):
            ...
            print("Time4: " + str(time.time() - starttime), file=sys.stderr)
            
        update_nonstars(self.current_image,self.source)
        print("Time5: "+str(time.time() - starttime), file=sys.stderr)
        if self.current_image.match is not None:
            self.last_match = self.current_image
        else:
            self.last_match=None
        print("Time6: {}".format(time.time() - starttime), file=sys.stderr)
    ...
    def extrapolate_image(self, imagefile1, imagefile2, time1, time2):
        ...
        print(a1, a2, np.linalg.svd(a1)[1], np.linalg.svd(a1)[1], file=sys.stderr)
        a,angrate = extrapolate_matrix(a1, a2, time1, time2, time.time() * 1e6)
